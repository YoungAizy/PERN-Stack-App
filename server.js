...
app.post('/api/v1/auth/signup', middleware.middleware, async(req, res) => {
    if (req.body.pass) {
        ...
        db.query("INSERT INTO users(name, password, email) values($1, $2, $3) returning *",
            ...
            .catch(err => {console.log(err); res.send("Could not register user. Error uploading info")});
    }  
});
//UPDATE USER DETAILS
    ...
    try {
        if (req.body.newPassword) {
            ...
            "UPDATE users SET name = $1, email= $2, password=$3 where email= $4 returning *",
                ...
                if (result.rowCount>0) {
                    const user = { name: req.body.name, email: req.body.newEmail };
                    const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET);
                    res.json({accessToken: accessToken, user: user, data: result.rows[0]});
                }
            }
        } else {
            const response = await db.query(
             ...
             if (response.rowCount>0) {
                    const user = { name: req.body.name, email: req.body.newEmail };
                    const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET);
                    res.json({accessToken: accessToken, user: user, data:response.rows[0]});
                }
        }
    } catch (error) {
        console.log(error);
...
app.delete("/api/v1/auth/:email", (req, res) => {
    db.query("DELETE FROM users WHERE email=$1", [escape(req.params.email).toLocaleLowerCase()]).
        then(response => { console.log(response); res.send("Deleted!")})
...
app.post("/api/v1/auth/login", middleware.middleware,(req, res) => {
    try {
            if (bcrypt.compareSync(req.body.password, req.hashedPassword)) {
                const user = {name: req.name, email: req.body.email}
                const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET);
...
app.get("/:look", async(req,res)=>{ 
    try {
        ...
        "select * from restaurants left join (select restaurant_id, COUNT(*), TRUNC(AVG(rating),1)as average_rating from reviews group by restaurant_id ) reviews on restaurants.id=reviews.restaurant_id LEFT JOIN images ON restaurants.id=images.restaurant where restaurants.name LIKE $1;",
             [`%${escape(req.params.look).toLowerCase()}%`]);
